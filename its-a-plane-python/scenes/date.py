...
class DateScene(object):
    ...
    def date(self, count):
        if len(self._data):
            ...
            # then draw the date
            now = datetime.now()
            current_date = now.strftime("%a, %b %d,%Y")  # Change the date format
            ...
            if self._last_date != current_date:
                ...
                if not self._last_date is None:
                    ...
                    self.draw_square(
                        0,
                        11,  # Start from the bottom of the screen (32 - 20)
                        17,  # Width of the area
                ...
                start_x = ((0 + 64) // 2) - date_string_width // 2
                DATE_POSITION = (start_x, 17)
