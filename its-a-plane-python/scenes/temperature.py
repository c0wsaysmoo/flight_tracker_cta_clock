...
# Scene Setup
TEMPERATURE_REFRESH_SECONDS = 630
TEMPERATURE_FONT = fonts.extrasmall

# Configure logging
#logging.basicConfig(filename='/home/flight/its-a-plane-python/temperature.log', level=logging.DEBUG, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    ...
    def temperature(self, count):
        ...
        if not (seconds_since_update % TEMPERATURE_REFRESH_SECONDS) or self._redraw_temp:
            if self._cached_temp is not None and self._redraw_temp:
                ...
                #logging.debug("Making API call for temperature and humidity data...")
                self._cached_temp = current_temperature, current_humidity = grab_temperature_and_humidity()
                self._last_updated = datetime.now()

                # Log the retrieved data
                #logging.debug(f"Temperature data: {current_temperature}")
                #logging.debug(f"Humidity data: {current_humidity}")
